...
public class NewApiClient {
    ...
    @Value("${newapi.whisper-model:gpt-4o-transcribe}")
    ...
    /**
     * 下载公网图片 URL 并转为 data URI（data:image/xxx;base64,...）。
     * 用于 chatCompletionWithImages 中把图片 URL 转成 base64 内嵌格式，
     * 应对 NewAPI 中转服务器无法访问公网 URL 的场景。
     */
    private String downloadAsDataUri(String imageUrl) {
        try {
            byte[] bytes = new java.net.URI(imageUrl).toURL().openStream().readAllBytes();
            ...
            throw new RuntimeException("下载图片转 data URI 失败: " + e.getMessage(), e);
        }
    }
    ...
    private SubmitResult submitVideoMultiImage(String prompt, byte[] imageBytes, String imageFilename,
        ...
        try {
            ...
            // 2026-08-14 修复:优先用 id(NewAPI 轮询用的 ID)而不是 task_id。
            // NewAPI poll 接口 /v1/videos/{id} 使用 id 字段,task_id 在轮询响应中可能变成不同的值。
    ...
    public SubmitResult submitVideoFull(String prompt, byte[] imageBytes, String imageFilename,
        ...
        VideoOptions opts = VideoOptions.builder()
            ...
            .ratio(ratio)
            .imageFilename(imageFilename != null ? imageFilename : "canvas_input.png")
            .imageMime(imageMime != null ? imageMime : "image/png")
            .build();
    ...
    public SubmitResult submitVideo(String prompt, List<byte[]> imageFiles, VideoOptions options) {
        ...
        try {
            ...
            // 从 imageFiles 提取主图字节（兼容旧代码中 imageBytes 变量名）
            byte[] imageBytes = (imageFiles != null && !imageFiles.isEmpty()) ? imageFiles.get(0) : null;
            ...
            if (imageBytes != null && imageBytes.length > 0) {
                ...
                final String fname = (options.getImageFilename() != null && !options.getImageFilename().isBlank())
                    ? options.getImageFilename() : "canvas_input.png";
                final String mime = (options.getImageMime() != null && !options.getImageMime().isBlank())
                    ? options.getImageMime() : "image/png";
                byte[] primaryImage = imageFiles.get(0);
                builder.part("input_reference",
                    new ByteArrayResource(primaryImage) {
                        @Override
            ...
            log.info("NewAPI video task submitted: {} (image={}, size={}B, duration={}s, resolution={})",
                taskId,
                imageFiles != null && !imageFiles.isEmpty() ? "canvas_input" : "placeholder",
    ...
    /**
     * 2026-08-14 新增:文生视频专用 — 按聚融中转站接口手册 v3.0 §6.2 严格走 JSON 格式。
     * 不传 image_url / input_reference,避免 aicoming-video-proxy 因缺 multipart 报错。
     *
     * <p>调用链:Java → NewAPI(/v1/videos, JSON body) → doubao-seedance
     *
     * <p>与 submitVideo 的区别:
     * <ul>
     *   <li>submitVideo 是 multipart,文生视频时必须传占位图(3 个字段名)</li>
     *   <li>本方法直接发 JSON,不传任何图片字段</li>
     * </ul>
     *
     * @param prompt     用户输入提示词
     * @param duration   视频时长(秒,如 4)
     * @param resolution 分辨率(如 "480p" / "720p" 小写)
     * @return           NewAPI 返回的 taskId 和(可能)直接返回的 video url
     */
    public SubmitResult submitVideoText(String prompt, int duration, String resolution) {
        final String useModel = "doubao-seedance-2.0";
        ...
        // 文生视频不传 image_url / input_reference

        log.info("NewAPI /v1/videos (文生视频, JSON): model={}, duration={}s, resolution={}, promptLen={}",
            useModel, duration, useResolution, prompt != null ? prompt.length() : 0);
        ...
        try {
            JsonNode response = webClientBuilder.baseUrl(baseUrl).build()
                ...
                .onErrorMap(WebClientResponseException.class, e -> {
                    String respBody = e.getResponseBodyAsString();
                    log.error("NewAPI /v1/videos (文生视频) failed: {} body={}",
                    ...
                    ErrorCode code = (e.getStatusCode().is4xxClientError())
                        ...
                        : ErrorCode.NEWAPI_UNREACHABLE;
                    return new BusinessException(code,
                        "NewAPI /v1/videos (文生视频) " + e.getStatusCode() + ": " + friendly);
                })
                .block();

            if (response == null) {
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "文生视频提交返回空");
            }
            // 2026-08-14 修复:优先用 id(NewAPI 轮询用的 ID)而不是 task_id
            String taskId = response.path("id").asText("");
            if (taskId.isEmpty()) {
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                    "NewAPI /v1/videos (文生视频) 响应缺少 id/task_id: " + response);
            }
            // 提取响应中的 video url(如果有)
            String directUrl = null;
            JsonNode topUrlNode = response.get("url");
            if (topUrlNode != null && topUrlNode.isTextual() && !topUrlNode.asText().isBlank()) {
                directUrl = topUrlNode.asText();
            ...
            log.info("NewAPI 文生视频 task 提交成功: taskId={}, directUrl={}",
                taskId, directUrl != null ? "yes" : "no");
            return new SubmitResult(taskId, directUrl);
        } catch (Exception e) {
            if (e instanceof BusinessException) throw e;
            log.error("NewAPI submitVideoText failed: {}", e.getMessage());
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, e.getMessage());
        }
    }
    ...
    /**
     * 2026-08-14 新增:画布模块专用图生图(单图/多图同接口 /v1/images/generations + JSON image 字段)
     * 与 editImage 的区别:
     *   - editImage() 走 /v1/images/edits multipart(OpenAI i2i 风格,中转站报 invalid multipart)
     *   - 本方法走 /v1/images/generations JSON(聚融中转站文档 5.4 推荐)
     * 支持:
     *   - 单图:referenceImages = ["url1"]
     *   - 多图:referenceImages = ["url1", "url2", "url3"] (aicoming 上游自动识别)
     *
     * @param model           模型名(如 gpt-image-2-2k)
     * @param prompt          生成提示词
     * @param size            图片尺寸(默认 1024x1024)
     * @param referenceImages 参考图 URL 列表(可为单张或多张)
     * @return 生成的图片(data URI / URL)
     */
    public String imageToImage(String model, String prompt, String size, java.util.List<String> referenceImages) {
        if (!checkHealth()) {
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 服务不可用，请稍后再试");
        }
        if (referenceImages == null || referenceImages.isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_PARAM, "图生图至少需要 1 张参考图");
        }

        final String useModel = (model != null && !model.isBlank()) ? model : "gpt-image-2-2k";
        Map<String, Object> body = new HashMap<>();
        body.put("model", useModel);
        body.put("prompt", prompt);
        body.put("size", size != null ? size : "1024x1024");
        body.put("response_format", "b64_json");
        body.put("n", 1);
        // 2026-08-14 用户最终方案:不走素材库、不走 MinIO URL。
        //   图片元素直接放 data URI (data:image/jpeg;base64,...) 在 HTTP body 里。
        //   NewAPI 中转不需要主动下载(它直接从 HTTP body 拿图片)。
        //   这样:
        //     - 不走 NewAPI 素材库(asset://)
        //     - 不依赖中转站访问 MinIO (192.140.163.161:19000 → 192.140.163.161:3000 网络不可达)
        //     - 图片直接走 HTTP body,中转站肯定能拿到
        // 接口手册 v3.0 §5.4:
        //   单图: "image": "url1"
        //   多图: "image": ["url1", "url2", ...]  (中转站会翻译成 images:[{image_url:...}] 给上游)
        ...
        log.info("NewAPI /v1/images/generations (图生图): model={}, refs={}, promptLen={}",
            useModel, referenceImages.size(), prompt != null ? prompt.length() : 0);
        // 2026-08-14:打印完整 body 帮助验证 §5.4 多图输入格式
        try {
            String bodyJson = new com.fasterxml.jackson.databind.ObjectMapper()
                .writeValueAsString(body);
            // 截短 prompt 防止日志爆炸
            String shortJson = bodyJson;
            if (bodyJson.length() > 600) {
                shortJson = bodyJson.substring(0, 600) + "...(prompt truncated, totalLen=" + bodyJson.length() + ")";
            }
            log.info("[imageToImage] request body: {}", shortJson);
        ...
        try {
            JsonNode response = webClientBuilder.baseUrl(baseUrl).build()
                ...
                .onErrorMap(WebClientResponseException.class, e -> {
                    log.error("NewAPI /v1/images/generations (图生图, model={}) failed: {} {}",
                        useModel, e.getStatusCode(), e.getResponseBodyAsString());
                    return new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                        "NewAPI 调用失败:" + e.getStatusCode() + " " + e.getStatusText());
                })
                .onErrorMap(e -> e instanceof BusinessException ? e :
                    new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, e.getMessage()))
                .block();

            if (response == null) {
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "图生图返回为空（响应为 null）");
            }
            if (!response.has("data")) {
                if (response.has("url") && response.get("url").isTextual()) return response.get("url").asText();
                if (response.has("b64_json") && response.get("b64_json").isTextual())
                    return "data:image/png;base64," + response.get("b64_json").asText();
                String errMsg = response.has("error") ? response.get("error").toString() : "Image-to-image failed";
                log.error("NewAPI imageToImage 失败: {}, response={}", errMsg, response.toString());
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 图生图失败:" + errMsg);
            }
            JsonNode dataArray = response.get("data");
            if (!dataArray.isArray() || dataArray.size() == 0) {
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "图生图 data 数组为空");
            }
            JsonNode first = dataArray.get(0);
            String[] urlFields = {"url", "image_url", "imageUrl", "img_url", "imgUrl"};
            for (String field : urlFields) {
                if (first.has(field) && first.get(field).isTextual()) {
                    log.info("NewAPI imageToImage OK ({}): {}", field, first.get(field).asText());
                    return first.get(field).asText();
                }
            }
            if (first.has("b64_json") && first.get("b64_json").isTextual()) {
                String b64Data = first.get("b64_json").asText();
                log.info("NewAPI imageToImage OK (b64_json): b64Len={}", b64Data.length());
                return "data:image/png;base64," + b64Data;
            }
            java.util.List<String> names = new java.util.ArrayList<>();
            first.fieldNames().forEachRemaining(names::add);
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 图生图 data[0] 缺少字段:" + names);
        } catch (Exception e) {
            if (e instanceof BusinessException) throw e;
            log.error("NewAPI imageToImage failed: {}", e.getMessage());
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 图生图调用失败:" + e.getMessage());
        }
    }

    /**
     * 画布模块专用：可指定模型名的文生图
     * @param model 模型名 (如 gpt-image-2-1k)
     */
    public String generateImageWithModel(String model, String prompt, String size, String quality, String style) {
        if (!checkHealth()) {
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 服务不可用，请稍后再试");
        }
        final String useModel = (model != null && !model.isBlank()) ? model : "gpt-image-2-1k";
        Map<String, Object> body = new HashMap<>();
        body.put("model", useModel);
        body.put("prompt", prompt);
        body.put("size", size != null ? size : "1024x1024");
        ...
        try {
            JsonNode response = webClientBuilder.baseUrl(baseUrl).build()
                ...
                .timeout(Duration.ofSeconds(300))
                .onErrorMap(WebClientResponseException.class, e -> {
                    log.error("NewAPI /v1/images/generations (model={}) failed: {} {}",
                        useModel, e.getStatusCode(), e.getResponseBodyAsString());
                    return new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                        "NewAPI 调用失败:" + e.getStatusCode() + " " + e.getStatusText());
                })
            ...
            if (!response.has("data")) {
                ...
                if (response.has("b64_json") && response.get("b64_json").isTextual())
                    return "data:image/png;base64," + response.get("b64_json").asText();
                String errMsg = response.has("error") ? response.get("error").toString() : "Image generation failed";
                log.error("NewAPI generateImageWithModel 失败: {}, response={}", errMsg, response.toString());
                throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI generateImage 失败:" + errMsg);
            }
            JsonNode dataArray = response.get("data");
            if (!dataArray.isArray() || dataArray.size() == 0) {
            ...
            java.util.List<String> names = new java.util.ArrayList<>();
            first.fieldNames().forEachRemaining(names::add);
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI data[0] 缺少字段:" + names);
        } catch (Exception e) {
            if (e instanceof BusinessException) throw e;
            log.error("NewAPI generateImageWithModel failed: {}", e.getMessage());
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE, "NewAPI 调用失败:" + e.getMessage());
        }
    }
     ...
     * 兼容旧调用方：仅传 audioBytes + mimeType，文件名自动推断。
     *
     * @param audioBytes 音频/视频二进制（wav/mp3/mp4 等）
     * @param mimeType   MIME 类型，如 audio/wav / video/mp4
    ...
    public List<Map<String, Object>> audioTranscribe(byte[] audioBytes, String mimeType) {
        ...
        if (mimeType != null) {
            ...
            else if (mimeType.contains("mp3")) ext = "mp3";
            else if (mimeType.contains("wav")) ext = "wav";
        }
        return audioTranscribe(audioBytes, "audio." + ext, mimeType);
    }

    /**
     * 音频/视频转文字（ASR），调 NewAPI /v1/audio/transcriptions。
     * 支持mp4视频直传（aicoming上游自动按音轨处理，无需先抽音轨）。
     *
     * @param fileBytes 文件二进制（wav/mp3/mp4 等）
     * @param filename  文件名（含后缀，如 demo.mp4 / audio.wav）
     * @param mimeType  MIME 类型，如 audio/wav / video/mp4
     * @return 识别结果列表，每段含 start (秒), end (秒), text (文本)
     */
    public List<Map<String, Object>> audioTranscribe(byte[] fileBytes, String filename, String mimeType) {
        if (fileBytes == null || fileBytes.length == 0) {
            log.warn("[ASR] fileBytes 为空，返回空列表");
            return List.of();
        }
        MultipartBodyBuilder builder = new MultipartBodyBuilder();
        builder.part("model", whisperModel);
        builder.part("language", "zh");
        // whisper-1 支持 verbose_json/srt/vtt（返回segments分段时间戳）
        // gpt-4o-transcribe / gpt-4o-mini-transcribe 只支持 json/text（只返回 text 字段）
        boolean useVerboseJson = "whisper-1".equalsIgnoreCase(whisperModel);
        if (useVerboseJson) {
            builder.part("response_format", "verbose_json");
        }
        final String finalFilename = filename != null ? filename : "audio.wav";
        ...
        try {
            JsonNode resp = webClientBuilder.baseUrl(videoBaseUrl).build()
                .post()
                .uri("/v1/audio/transcriptions")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.MULTIPART_FORM_DATA)
            ...
            log.info("[ASR] 原始响应: {}", resp != null ? resp.toString().substring(0, Math.min(resp.toString().length(), 500)) : "null");

            // 优先解析 segments 数组（verbose_json 格式）
            ...
            if (segments != null && segments.isArray() && segments.size() > 0) {
                ...
                log.info("[ASR] 识别完成(segments): {} segments, {} bytes", result.size(), fileBytes.length);
                return result;
            }

            // 兜底:如果没有 segments，尝试解析 text 字段（普通格式）
            JsonNode textNode = resp != null ? resp.path("text") : null;
            if (textNode != null && !textNode.isMissingNode()) {
                String fullText = textNode.asText("");
                if (fullText != null && !fullText.isBlank()) {
                    Map<String, Object> single = new HashMap<>();
                    single.put("start", 0.0);
                    single.put("end", 30.0);
                    single.put("text", fullText);
                    log.info("[ASR] 识别完成(text 兜底): {} 字符, {} bytes", fullText.length(), fileBytes.length);
                    return List.of(single);
                }
            }

            log.warn("[ASR] 无可用文本: segments 为空且 text 字段缺失, resp={}", resp);
            return List.of();
        } catch (Exception e) {
    ...
    public SubmitResult submitVideoByAssetRefList(String prompt, java.util.List<String> imageUrls,
        ...
        // 2026-08-15 修复:严格按文档 §6.3 只用 id 字段,不 fallback 到 task_id。
        // NewAPI poll 接口 /v1/videos/{id} 使用 id 字段,task_id 是 aicoming 内部 ID,两者不同。
        String taskId = response.path("id").asText("");
    ...
    /**
     * 2026-08-15 新增:多图生视频(references[] 格式) — 严格按聚融中转站接口手册 v3.0 §6.4。
     *
     * <p>与 {@link #submitVideoByAssetRefList} 的区别:
     * <ul>
     *   <li>后者:用 image_url 单数字段,只取第一张图</li>
     *   <li>本方法:用 references[] 数组,支持多张图,每张图可设 alias 让 prompt 引用不同角色/场景</li>
     * </ul>
     *
     * <p>请求体(手册 §6.4):
     * <pre>{@code
     * {
     *   "model": "doubao-seedance-2.0",
     *   "prompt": "Alice 操作控制台, Bob 看全息图",
     *   "references": [
     *     {"media_type": "image", "url": "asset://aic_Alice", "alias": "Alice"},
     *     {"media_type": "image", "url": "asset://aic_Bob", "alias": "Bob"}
     *   ],
     *   "duration": 4,
     *   "resolution": "480P",
     *   "generate_audio": true
     ...
     * }</pre>
     *
     * @param prompt    用户提示词(可在其中用 alias 引用不同图,如"Alice 走向 Bob")
     * @param assetUrls 已上传到素材库的 asset_url 列表(asset://aic_xxx),1-4 张
     * @param aliases   每张图的别名(可空,空时自动生成 image_1/image_2/...)
     * @param duration  时长(秒),默认 4
     * @param resolution 分辨率,如 480p / 720p
     * @return SubmitResult(taskId, url)
     */
    public SubmitResult submitVideoByReferences(String prompt,
                                                  java.util.List<String> assetUrls,
                                                  java.util.List<String> aliases,
                                                  int duration, String resolution) {
        if (assetUrls == null || assetUrls.isEmpty()) {
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                "submitVideoByReferences: assetUrls 为空,无法提交");
        }
        if (assetUrls.size() > 4) {
            log.warn("[VIDEO-REF] 收到 {} 张图,文档 §6.4 最多 4 张,截取前 4 张", assetUrls.size());
            assetUrls = assetUrls.subList(0, 4);
        }

        // 2026-08-15 修改:用 image_urls[] 字符串数组格式(用户指定),不用 references[] 对象数组
        //   单图用 image_url,多图必须用 image_urls[],两者不能共存
        //   参考用户示例:{"image_urls": ["asset://aic_图1", "asset://aic_图2"]}
        Map<String, Object> body = new HashMap<>();
        body.put("model", "doubao-seedance-2.0");
        body.put("prompt", prompt == null ? "" : prompt);
        body.put("image_urls", new ArrayList<>(assetUrls));
        body.put("duration", String.valueOf(duration));
        body.put("resolution", resolution == null ? "480p" : resolution.toLowerCase());

        log.info("[VIDEO-REF] → POST {}/v1/videos (image_urls[]): images={}, aliases={}, duration={}s, resolution={}, promptLen={}",
            videoBaseUrl, assetUrls.size(),
            aliases != null ? aliases : "(auto)",
            duration, resolution, prompt == null ? 0 : prompt.length());
        ...
        try {
                ...
                .exchangeToMono(clientResponse -> {
                    ...
                    if (status >= 400) {
                        return clientResponse.bodyToMono(String.class)
                            .defaultIfEmpty("")
                            .map(bodyStr -> {
                                log.warn("[VIDEO-REF] /v1/videos 收到 HTTP {}: {}", status,
                                    bodyStr.length() < 500 ? bodyStr : bodyStr.substring(0, 500) + "...");
                                try {
                                    if (bodyStr != null && !bodyStr.isBlank()) {
                                        return new ObjectMapper().readTree(bodyStr);
                                    }
                                } catch (Exception parseEx) {
                                    log.warn("[VIDEO-REF] body 不是 JSON: {}", parseEx.getMessage());
                                }
                                return null;
                            });
                    }
            ...
            submitException = e;
            log.error("[VIDEO-REF] /v1/videos 抛异常: {} ({}ms)", e.getMessage(),
                System.currentTimeMillis() - submitStart);
        }

        long submitElapsed = System.currentTimeMillis() - submitStart;
        log.info("[VIDEO-REF] /v1/videos 提交完成,耗时 {}ms", submitElapsed);
        ...
        if (response == null) {
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                "NewAPI /v1/videos (references[]) 返回空响应");
        }

        // 检测 queued 占位响应(id/task_id 为 null 但 status=queued)
        String respStatus = response.path("status").asText("");
        String respId = response.path("id").asText("");
        String respTaskId = response.path("task_id").asText("");
        if ((respId.isEmpty() && respTaskId.isEmpty())
            && ("queued".equalsIgnoreCase(respStatus) || "pending".equalsIgnoreCase(respStatus))) {
            log.warn("[VIDEO-REF] aicoming-proxy 返回 queued 占位响应 (id/task_id=null)");
            throw new BusinessException(ErrorCode.NEWAPI_REQUEST_INVALID,
                "aicoming 上游已入队但 id 尚未生成 (status=" + respStatus + "),请稍后重试或查询");
        }
        ...
        String taskId = response.path("id").asText("");
        if ("none".equalsIgnoreCase(taskId) || "null".equalsIgnoreCase(taskId)) {
            log.warn("[VIDEO-REF] 3000 中转站返回占位 taskId='{}',视为无效", taskId);
            taskId = "";
        }
        if (taskId.isEmpty()) {
            throw new BusinessException(ErrorCode.NEWAPI_UNREACHABLE,
                "NewAPI /v1/videos (references[]) 响应缺 id: " + response);
        }
        ...
        log.info("[VIDEO-REF] OK: taskId={}, images={}, directUrl={}", taskId, assetUrls.size(),
            directUrl != null ? "有" : "无");
        return new SubmitResult(taskId, directUrl);
    }
