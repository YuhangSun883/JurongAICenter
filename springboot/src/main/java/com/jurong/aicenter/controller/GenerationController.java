...
public class GenerationController {
    ...
    public JobResponse getJob(
        ...
        if (resultUrls != null && !resultUrls.isEmpty()) {
            String firstUrl = resultUrls.get(0);
            if (firstUrl != null && !firstUrl.isBlank()) {
                try {
                    String key = firstUrl.contains("?")
                        ? firstUrl.substring(firstUrl.lastIndexOf('/') + 1, firstUrl.indexOf('?'))
                        : firstUrl.substring(firstUrl.lastIndexOf('/') + 1);
                    String idStr = key.replaceAll("[^0-9]", "");
                    if (!idStr.isBlank()) {
                        mediaAssetId = Long.parseLong(idStr);
                    }
