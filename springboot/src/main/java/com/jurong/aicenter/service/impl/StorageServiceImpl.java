...
public class StorageServiceImpl implements StorageService {
    ...
    /**
     * 2026-08-14 新增:批量删除 MinIO 对象(用于磁盘满了紧急清理)。
    ...
    public int deleteFiles(List<String> objectKeys) {
        ...
        for (String key : objectKeys) {
            try {
                ...
                log.warn("MinIO batch delete failed: key={}, err={}", key, e.getMessage());
            }
        }
        log.info("[storage] batch delete done: requested={}, ok={}", objectKeys.size(), ok);
        return ok;
    }

    /**
     * 2026-08-14 新增:列出 bucket 里所有对象(分页),用于后台清理任务或管理端点。
     * 注意:MinIO listObjects 单次 maxKeys 限制 1-1000,需要循环分页。
     */
    @Override
    public List<String> listAllObjectKeys(int maxKeys) {
        List<String> keys = new ArrayList<>();
        // MinIO 单次最多 1000,所以分页循环
        int pageSize = Math.min(Math.max(maxKeys, 1), 1000);
        int collected = 0;
        try {
            while (collected < maxKeys) {
                int batch = Math.min(pageSize, maxKeys - collected);
                ...
                for (Result<Item> r : results) {
                    try {
                        ...
                        log.warn("[storage] list item parse failed: {}", e.getMessage());
                    }
                }
                // 如果这页没拿到数据,说明已经到末尾
                if (gotThisPage == 0) break;
                if (gotThisPage < batch) break;
            }
        } catch (Exception e) {
            log.error("[storage] listAllObjectKeys failed: {}", e.getMessage());
        }
        return keys;
    }
