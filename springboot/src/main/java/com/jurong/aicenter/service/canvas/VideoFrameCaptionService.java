...
public class VideoFrameCaptionService {
    ...
    public void executeCaptionAsync(CanvasTask task, CanvasNode node,
        ...
        try {
            ...
            // 接口文档 2026-08-14: mp4 视频直传 /v1/audio/transcriptions,无需先抽音轨
            String fullDubText = "";
            try {
                byte[] videoBytes = null;
                String filename = "video.mp4";
                // 优先:直接下载 mp4 视频字节(无需 ffmpeg 抽音轨)
                try {
                    videoBytes = new java.net.URI(videoUrl).toURL().openStream().readAllBytes();
                    // 从 URL 提取文件名后缀
                    String lowerUrl = videoUrl.toLowerCase();
                    if (lowerUrl.contains(".mp4")) filename = "video.mp4";
                    else if (lowerUrl.contains(".webm")) filename = "video.webm";
                    else if (lowerUrl.contains(".mov")) filename = "video.mov";
                    log.info("[video-caption] 视频下载完成: {} bytes, 准备直传 ASR", videoBytes.length);
                } catch (Exception downloadErr) {
                    // 兜底:URL 下载失败时,退回 ffmpeg 抽 wav 音轨
                    log.warn("[video-caption] 视频直传下载失败,退回 ffmpeg 抽音轨: {}", downloadErr.getMessage());
                    Path tempVideoDir = frames.get(0).path().getParent();
                    Path audioFile = tempVideoDir.resolve("audio.wav");
                    Path extracted = extractor.extractAudio(videoUrl, audioFile);
                    if (extracted != null) {
                        videoBytes = java.nio.file.Files.readAllBytes(extracted);
                        filename = "audio.wav";
                    }
                }
                if (videoBytes != null && videoBytes.length > 0) {
                    String mimeType = filename.endsWith(".wav") ? "audio/wav" : "video/mp4";
                    List<Map<String, Object>> segments = newApiClient.audioTranscribe(videoBytes, filename, mimeType);
                    // 直接拼接所有 segments 的 text 作为完整口播原文
                    StringBuilder allDub = new StringBuilder();
                    for (Map<String, Object> seg : segments) {
                        String text = (String) seg.get("text");
                        if (text != null && !text.isBlank()) {
                            if (allDub.length() > 0) allDub.append(" ");
                            allDub.append(text);
                        }
                    }
                    fullDubText = allDub.toString();
                    log.info("[video-caption] ASR 完成: {} segments, {} 字符", segments.size(), fullDubText.length());
                }
            ...
            if (needCaption) {
                ...
                // 拼口播文案模板(运镜+动作+口播原文)
                content = assembleScript(frames, captions, fullDubText);
                node.setContent(content);
                task.setTextResult(content);
            } else {
                // frames 模式：不调 VL，但需要把口播原文写入文本节点
                StringBuilder sb = new StringBuilder();
                sb.append(String.format("已抽帧 %d 张，按 F5 查看", frames.size()));
                if (fullDubText != null && !fullDubText.isBlank()) {
                    sb.append("\n\n[口播原文-从Shot01第一秒立即开始，连贯贯穿全程] ")
                      .append(fullDubText);
                    log.info("[video-caption-frames] 口播原文拼接完成: {} 字符", fullDubText.length());
                }
                content = sb.toString();
                node.setContent(content);
                task.setTextResult(content);
     ...
     *   ...
     */
    private String assembleScript(List<FrameMeta> frames, List<Map<String, String>> captions, String fullDubText) {
        ...
        // 拼接口播原文(直接用 ASR 返回的完整文本)
        if (fullDubText != null && !fullDubText.isBlank()) {
            sb.append("\n\n[口播原文-从Shot01第一秒立即开始，连贯贯穿全程] ")
              .append(fullDubText);
            log.info("[video-caption] 口播原文拼接完成: {} 字符", fullDubText.length());
        }
