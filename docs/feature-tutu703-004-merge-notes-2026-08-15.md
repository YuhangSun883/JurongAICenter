...
- 新增前台页面：`/credits`。
- 页面展示可用积分、订阅积分、充值积分。
- 页面支持“兑换充值卡”和“订阅 / 充值”，复用现有充值弹窗能力。
- 新增当前用户自己的积分明细查询接口：`GET /api/agent/credits/ledger`。
- 明细支持按类型筛选：全部、消耗、退款、积分充值、过期、赠送。
- 明细支持按工具筛选：全部工具、视频生成、图片生成、视频拆条、画质增强、提示词生成、Agent 对话、充值/兑换。
- 该接口只读取当前登录用户自己的 `billing_logs`，不会暴露其他用户流水。
- 现阶段 `billing_logs` 未单独存 `tool` 字段，工具筛选按说明文字兼容匹配；后续正式计费建议增加独立 `tool_key/source_scene` 字段。
...
11. AI 视频前台时长上限为 15 秒，前端控件和后端 `/api/videos` 入参都会限制到 15 秒。
12. AI 视频“帮我写”点击后先展示预计积分弹窗，再进入上传产品/模特图、识别类型和名称、选择业务场景、生成脚本、一键生成视频流程。
